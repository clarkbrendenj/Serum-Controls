...
numeric_datetime = datetime.now().strftime("%Y_%m_%d__%H%M")
today = date.today()
...
#today_start = '2024-08-16 00:00:00'
parms = {"start_date": today_start, "end_date": current_datetime}

two_days_ago = datetime.now() - timedelta(days=2)

# Format the date and time as a string
m_start = two_days_ago.strftime('%Y-%m-%d %H:%M:%S')
...
AND (((uces.in_lab_dt_tm BETWEEN cclsql_cnvtdatetimeutc(to_date(:start_date,'YYYY-MM-DD HH24:MI:SS'),1,126,1) AND cclsql_cnvtdatetimeutc(to_date(:end_date,'YYYY-MM-DD HH24:MI:SS'),1,126,1))
    AND (container.spec_cntnr_cd IN (2552866269))
      AND (uces.institution_cd IN (2555515351.00)))
...
serum_df = pd.DataFrame(serum_result, columns=columns)
cur.close()
connection.close()
...
AND (uces.patient_id = per.person_id)
AND (((uces.in_lab_dt_tm BETWEEN cclsql_cnvtdatetimeutc(to_date(:start_date,'YYYY-MM-DD HH24:MI:SS'),1,126,1) AND cclsql_cnvtdatetimeutc(to_date(:end_date,'YYYY-MM-DD HH24:MI:SS'),1,126,1))
    AND (enc.loc_nurse_unit_cd IN (2552871925.00,2552871943.00,2552871949.00,2555187977.00,2554842765.00,2552871955.00,2555865409.00,2552871931.00,2554975363.00,2552877235.00,2555408411.00,2552877283.00))
...
filtered_serum_df = serum_df[~serum_df['mrn'].isin(micro_df['mrn'])]
filtered_serum_df

acc = filtered_serum_df[['patient_name', 'serum_accession_nbr', 'mrn', 'pt_location', 'Container Disp']]

unique_rows_df = acc.drop_duplicates()
row_count = len(unique_rows_df.index)
unique_rows_df.to_csv(
    f"//vpenslab/LabShared/MicroBiology/QMS Data/Serum Study Enroll Controls/Serum Study Enroll Controls {numeric_datetime}.csv",
    index=False,
)

body = f"<h2>Patient serum controls {today_start} to {current_datetime}</h2>"

send_email(
    to=[
        "clarkb@mskcc.org"
    ],
    subject=f"Patient serum controls {today_start} to {current_datetime}",
    body= f"number of rows: {row_count}",
